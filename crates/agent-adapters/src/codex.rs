...
impl CodexWorkspaceAgent {
    ...
    async fn invoke_inner(
        ...
        let head_before = match request.adopted_worktree_fingerprint.as_deref() {
            Some(expected) => ensure_adopted_worktree(&request.cwd, expected)?,
            None => ensure_clean_worktree(&request.cwd)?,
        };
        let mut stream = self
            .adapter
            .run(AgentRunRequest {
