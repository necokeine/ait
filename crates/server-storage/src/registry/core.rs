...
pub(super) struct FileRegistry<R> {
    path: PathBuf,
    state: Mutex<State<R>>,
...
impl<R: Clone + Serialize + DeserializeOwned> FileRegistry<R> {
    pub(super) fn new(path: PathBuf, id: fn(&R) -> &str) -> Self {
        Self {
            path,
    ...
    pub(super) fn initialize(&self) -> Result<(), RegistryError> {
        drop(self.loaded()?);
        Ok(())
    }
    ...
    pub(super) fn list(&self) -> Result<Vec<R>, RegistryError> {
        Ok(self.loaded()?.records.values().cloned().collect())
    }

    pub(super) fn get(&self, id: &str) -> Result<Option<R>, RegistryError> {
