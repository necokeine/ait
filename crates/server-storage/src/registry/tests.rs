...
fn project_lifecycle_preserves_records_and_publishes_committed_changes_only() {
    ...
    let listener: server_ports::registry::MutationListener<
        server_ports::registry::ProjectMutation,
    > = Arc::new(move |event| {
...
fn active_root_allocation_serializes_and_keeps_archived_and_legacy_ids() {
    ...
    refreshed_input.kind = server_domain::registry::PersistedProjectKind::NonGit;
...
fn workspace_lifecycle_keeps_distinct_ids_at_one_cwd_and_archive_context() {
    use server_ports::registry::{WorkspaceArchiveContext, WorkspaceMutationContext};
    let temp = tempfile::tempdir().unwrap();
