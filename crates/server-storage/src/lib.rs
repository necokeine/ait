...
fn open(
    ...
    let path = path
        ...
        .join(path.file_name().ok_or(ProjectError::Invalid)?);
...
fn sql(error: &rusqlite::Error) -> ProjectError {
    match error.sqlite_error_code() {
        ...
        Some(rusqlite::ErrorCode::ConstraintViolation) => ProjectError::IdentityConflict,
        _ => ProjectError::Io,
    }
}
