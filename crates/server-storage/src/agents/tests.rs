...
use server_ports::{Catalog, ProjectError};
...
#[test]
fn v1_upgrade_preserves_project_receipts_and_retains_reopenable_backup() {
    let directory = tempfile::tempdir().unwrap();
    let mut original =
        SqliteCatalog(Connection::open(directory.path().join("catalog.sqlite3")).unwrap());
    original.0.execute_batch(crate::catalog::SCHEMA).unwrap();
    original
        .0
        .execute_batch("PRAGMA application_id=1095979843; PRAGMA user_version=1;")
        .unwrap();
    let path = directory.path().join("project");
    let intent = original.begin_open("open", &path).unwrap();
    let entry = server_ports::CatalogEntry {
        id: server_domain::ProjectId::generate(),
        path: path.clone(),
        name: "preserved".to_owned(),
        base_commit: "a".repeat(40).parse().unwrap(),
        root_message_id: server_domain::MessageId::generate(),
        created_at: 42,
    };
    let receipt = original.finish_open(&intent, &entry).unwrap();
    let pending = original.begin_open("pending", directory.path()).unwrap();
    drop(original);
    let mut migrated = SqliteCatalog::open(directory.path()).unwrap();
    assert_eq!(
        migrated.begin_open("open", &path).unwrap().operation_id,
        intent.operation_id
    );
    assert_eq!(
        migrated.begin_open("open", &path).unwrap().receipt,
        Some(receipt)
    );
    assert_eq!(migrated.get(entry.id).unwrap(), entry);
    let resumed = migrated.begin_open("pending", directory.path()).unwrap();
    ...
    migrated.configure(&create("create")).unwrap();
    drop(migrated);
    ...
    SqliteCatalog::open(directory.path()).unwrap();
...
fn migration_failure_rolls_back_version_and_foreign_formats_are_untouched() {
    ...
    old.execute_batch(crate::catalog::SCHEMA).unwrap();
