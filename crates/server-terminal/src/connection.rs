...
#[derive(Default)]
/// Terminal output, listing and resize ownership for one physical connection.
...
impl TerminalConnection {
    /// Count terminal output and listing subscriptions.
    #[must_use]
    pub fn len(&self) -> usize {
        self.streams.len() + self.lists.len()
    }

    /// Whether no terminal subscriptions remain.
    #[must_use]
    pub fn is_empty(&self) -> bool {
        self.len() == 0
    }

    /// Release a terminal observer owned by this connection.
        ...
        let result = match method.as_str() {
            ...
            _ => {
                run(state, move |service| {
                    crate::rpc::execute(service, &method, params)
                })
                .await
            }
        };
    ...
    /// Apply a terminal input event with this connection's resize identity.
    /// # Errors
    /// Rejects invalid payloads, unavailable terminals or runtime admission failures.
    pub async fn event(&mut self, params: Value, state: &Shared) -> Result<(), ErrorCode> {
        let request: wire::InputRequest = decode(params)?;
        self.input(request.terminal_id, request.message, state)
            .await
    }

    /// Decode an input frame and apply it to this connection's terminal slot.
    /// # Errors
    /// Rejects malformed frames, unknown slots or terminal/runtime failures.
    pub async fn binary(&mut self, bytes: &[u8], state: &Shared) -> Result<(), ErrorCode> {
    ...
    /// # Errors
    /// Returns encoding or queue failures.
    pub async fn poll(&mut self, state: &Shared, outbound: &Outbound) -> Result<(), QueueError> {
        if self.is_empty() {
            return Ok(());
        }
...
pub(crate) async fn run<R: Send + 'static>(
    ...
    let job = tokio::task::spawn_blocking(move || {
        ...
        execute(&mut *service.lock().map_err(|_| crate::Error::Io)?)
    });
    drop(admission);
    job.await
...
pub(crate) fn send_error(outbound: &Outbound, code: ErrorCode) -> Result<(), QueueError> {
...
pub fn maintain(state: &std::sync::Arc<Shared>) {
