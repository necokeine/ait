...
use server_application::files::{self as port, Files};
use server_protocol::{ErrorCode, files as wire};
use uuid::Uuid;
...
mod connection;
mod transfer;
pub(super) use connection::FileConnection;
pub(super) use connection::FileRequest;
...
async fn execute(method: String, params: Value, state: &Shared) -> Result<Value, ErrorCode> {
    crate::jobs::run(
        ...
        state.files.clone(),
        ErrorCode::ProjectIo,
        move |files| dispatch(files, &method, params),
    )
    .await
}

fn dispatch(files: &mut Files, method: &str, params: Value) -> Result<Value, ErrorCode> {
    match method {
...
fn project_version(cwd: &str, path: &str, version: port::FileVersion) -> wire::FileVersion {
    match version {
