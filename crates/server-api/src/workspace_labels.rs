...
use server_protocol::workspace_labels::{
...
impl PendingSubscription {
    pub fn activate(self) -> Result<(String, WorkspaceLabelSubscription), QueueError> {
...
impl Delivery {
    ...
    fn send(&self, change: &SequencedWorkspaceLabelChange) -> Result<(), QueueError> {
        let params = serde_json::to_value(live_update(&self.subscription_id, change))?;
        self.outbound.send(&ServerMessage::Event {
...
pub(super) async fn dispatch(
    ...
    crate::jobs::run(
        ...
        move |labels| execute(labels, &method, params, outbound),
    )
    .await
}
