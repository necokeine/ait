...
use server_filesystem::rpc::checkout::DiffObservation;
use server_filesystem::service::checkout::{self as port, Checkout};
...
pub(super) async fn dispatch(
    ...
    crate::jobs::run(
        ...
        ErrorCode::ProjectIo,
        move |checkout| {
            let value = server_filesystem::rpc::checkout::execute(checkout, &method, params)?;
            Ok(Dispatch {
                value,
                subscription: None,
            })
...
async fn subscribe(
    ...
    let (observation, value) = crate::jobs::run(
        state,
        state.checkout.clone(),
        ErrorCode::ProjectIo,
