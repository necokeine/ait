...
use server_application::agent_runtime::{
...
use server_protocol::agent_lifecycle::{
...
use server_protocol::workspace::{ProjectCheckoutLitePayload, ProjectPlacementPayload};
...
fn list(directory: &AgentRuntimeDirectory, request: AgentListRequest) -> Result<Value, ErrorCode> {
    ...
    let query = query(
        ...
        request.sort,
        request.page,
        request.scope.as_deref() == Some("active"),
        false,
        None,
    )?;
    encode(page(
...
fn query(
    ...
    page: Option<server_protocol::agent_lifecycle::AgentPageRequest>,
    active_scope: bool,
    history_defaults_to_archived: bool,
    ...
    Ok(AgentDirectoryQuery {
        ...
        include_archived: filter
            .include_archived
            .unwrap_or(history_defaults_to_archived),
...
fn snapshot(record: &PersistedAgentRuntimeRecord) -> AgentSnapshotPayload {
...
) -> server_protocol::agent_lifecycle::AgentAttentionReason {
    match reason {
        DomainAttentionReason::Finished => {
            server_protocol::agent_lifecycle::AgentAttentionReason::Finished
        }
        DomainAttentionReason::Error => {
            server_protocol::agent_lifecycle::AgentAttentionReason::Error
        }
        DomainAttentionReason::Permission => {
            server_protocol::agent_lifecycle::AgentAttentionReason::Permission
        }
    }
}
