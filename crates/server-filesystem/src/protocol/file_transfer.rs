...
/// Invalid or oversized binary file-transfer frame.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub enum FrameError {
    /// Identifier, metadata or chunk violates the wire framing limits.
    #[error("Invalid file transfer frame")]
    InvalidFrame,
}
...
pub fn decode(bytes: &[u8]) -> Option<(String, FileFrame)> {
    ...
    let request_id = std::str::from_utf8(bytes.get(2..2 + length)?)
        .ok()?
        .to_owned();
    if !crate::protocol::valid_id(&request_id) {
        return None;
    }
...
pub fn encode(request_id: &str, frame: &FileFrame) -> Result<Vec<u8>, FrameError> {
    let length = u8::try_from(request_id.len()).map_err(|_| FrameError::InvalidFrame)?;
    if !crate::protocol::valid_id(request_id) || length == 0 {
    ...
    match frame {
        FileFrame::Begin(metadata) => {
            let json = serde_json::to_vec(metadata).map_err(|_| FrameError::InvalidFrame)?;
            let length = u16::try_from(json.len()).map_err(|_| FrameError::InvalidFrame)?;
            bytes.extend_from_slice(&length.to_be_bytes());
            bytes.extend_from_slice(&json);
        }
        FileFrame::Chunk(chunk) if chunk.len() <= CHUNK_BYTES => bytes.extend_from_slice(chunk),
        FileFrame::Chunk(_) => return Err(FrameError::InvalidFrame),
        FileFrame::End => {}
    }
    Ok(bytes)
}
