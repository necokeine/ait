...
impl RunApproval for LocalControlService {
    async fn decide(&self, request: ApprovalRequest) -> Result<ApprovalDecision, DomainError> {
        ...
        self.request_api_tool_approval(request.execution, None, i64::MAX)
            .await
            .map_err(api_domain_error)
    }
    async fn consume(&self, grant: &ToolGrant) -> Result<bool, DomainError> {
        self.consume_api_tool_grant(grant, None)
            .await
            .map_err(api_domain_error)
    }
}
...
impl LocalControlService {
    ...
    pub(super) async fn request_api_tool_approval(
        ...
        self.wait_for_tool_approval(record, lease, receiver).await
    }
    ...
    pub(super) async fn consume_api_tool_grant(
        ...
        for _ in 0..8 {
            ...
            let run = state
                ...
                .find(|r| r.id == grant.run_id)
                .ok_or_else(invalid)?;
            if !active(run, lease) || run.lease_epoch != grant.lease_epoch {
                return Ok(false);
            }
            let Some(index) = run
                .tool_approvals
    ...
    pub async fn resolve_tool_approval(
        ...
        for _ in 0..8 {
            ...
            let live_waiter = self
                .tool_approval_waiters
                .lock()
                .map_err(|_| invalid())?
                .get(approval_id)
                .is_some_and(|w| w.run_id == run.id && w.epoch == run.lease_epoch);
