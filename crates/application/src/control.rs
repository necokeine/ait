...
impl LocalControlService {
        ...
        read.records
            ...
            .ok_or_else(|| match kind {
                ControlRecordKind::Session => {
                    error(ErrorCode::SessionNotFound, "session not found", false)
                }
                ControlRecordKind::Run => error(ErrorCode::InvalidRun, "run not found", false),
                ControlRecordKind::Cron => error(ErrorCode::InvalidCron, "cron not found", false),
                _ => error(
    ...
    async fn read_project_records(&self, project_id: &str) -> Result<LoadedWorkingSet, ApiError> {
        self.read_project_records_with(project_id, Vec::new()).await
    }

    async fn read_project_records_with(
        &self,
        project_id: &str,
        ...
        let mut filters = vec![
            ...
            ControlFilter::project(Kind::RunCredential, project_id),
            ControlFilter::project(Kind::Session, project_id),
            ControlFilter::project(Kind::Message, project_id),
            ControlFilter::project(Kind::Run, project_id),
            ControlFilter::project(Kind::WorkspaceRunJournal, project_id),
            ControlFilter::project(Kind::Cron, project_id),
    ...
    async fn read_catalog_records(&self) -> Result<LoadedWorkingSet, ApiError> {
        use ControlRecordKind as Kind;
    ...
    async fn read_session_records(&self, session_id: &str) -> Result<LoadedWorkingSet, ApiError> {
        let project_id = self
            .related_project_id(ControlRecordKind::Session, session_id)
            .await?;
        self.read_project_records(&project_id).await
    }

    async fn read_run_records(&self, run_id: &str) -> Result<LoadedWorkingSet, ApiError> {
        let project_id = self
            .related_project_id(ControlRecordKind::Run, run_id)
            .await?;
        self.read_project_records(&project_id).await
    }
    ...
    async fn read_cron_records(&self, cron_id: &str) -> Result<LoadedWorkingSet, ApiError> {
        let project_id = self
            .related_project_id(ControlRecordKind::Cron, cron_id)
            .await?;
        self.read_project_records(&project_id).await
    }
    ...
    async fn read_command_records(&self, command: &Command) -> Result<LoadedWorkingSet, ApiError> {
        ...
        match command {
            ...
            | Command::RefreshProviderModels { .. } => self.read_catalog_records().await,
            Command::ListAgents => {
            ...
            | Command::ListRuns { project_id } => self.read_project_records(project_id).await,
            Command::CreateSession {
                project_id,
                at_message_id,
                ..
            } => {
                self.read_project_records_with(
                    project_id,
                    at_message_id
                        .iter()
                        .map(|id| ControlFilter::id(Kind::Message, id))
                        .collect(),
                )
                .await
            }
            Command::ForkSession {
                ...
                self.read_project_records_with(
                    project_id,
                    vec![ControlFilter::id(Kind::Message, at_message_id)],
                )
                .await
            }
            ...
            Command::GetRun { run_id } | Command::CancelRun { run_id } => {
                self.read_run_records(run_id).await
            }
