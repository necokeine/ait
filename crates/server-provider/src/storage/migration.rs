...
pub(super) fn catalog(connection: &mut Connection, data_dir: &Path) -> Result<(), AgentError> {
    ...
    connection
        ...
        .map_err(|error| sql(&error))?;
    backup.as_file().sync_all().map_err(|_| AgentError::Io)?;
    let (_file, _path) = backup.keep().map_err(|_| AgentError::Io)?;
    #[cfg(unix)]
    ...
    let transaction = connection
        ...
        .map_err(|error| sql(&error))?;
    transaction
        .execute_batch(crate::storage::agents::SCHEMA)
        .map_err(|error| sql(&error))?;
    transaction
