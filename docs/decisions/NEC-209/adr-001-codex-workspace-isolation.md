...
5. 最终整合采用带补偿日志的锁定发布协议而非“检查后再 merge”：先完成可失败的隔离 worktree 清理，再 prepare 带旧 OID 的 Git ref transaction；attached HEAD 固定并直接更新准入时的精确 symbolic target ref，detached HEAD 固定为 no-deref `HEAD`，后续 branch switch 不得改变补偿目标。目标 ref 的 direct identity 必须在 prepared transaction 持有精确 ref lock 时确认，补偿 CAS 也先取得同一锁再确认 identity；外部改成的 symbolic ref 即使解析到 candidate OID 也必须原样保留。取得 canonical index lock 并复制准入 index，同时按 baseline/candidate 两棵 tree 的精确 entry/type 把会被 Run 改写的路径和目标 index 记录到 manager-owned rollback journal；在首次 `read-tree -u` 前，以 baseline index 的 ignore 视图检查 candidate 将创建或 D/F 替换的实际 filesystem 路径，碰到普通 untracked、仓库/exclude/global ignored 内容都拒绝，不能让 candidate 的新 ignore 规则掩盖碰撞。D/F 转换在补偿时先用尚未变动且包含 candidate `.gitattributes` 的一致 worktree 视图冻结全部路径的 Run ownership，再由深到浅撤下 candidate entry、由浅到深恢复 baseline entry。协议在 index lock 前、`read-tree` 前后、ref commit confirmation 前、ref publish 后及 index publish 前设置明确检查点，并反复校验 HEAD、branch、canonical index 原始字节、candidate index tree、tracked worktree 和 untracked files。ref commit 命令一经尝试即视为结果不确定；先完全结算 transaction 进程，再读取原始精确 target ref：仍为 candidate 且 identity 未变时以旧 OID CAS 撤回，已为 baseline 且 identity 未变时无需处理，其他 OID/identity 则保留外部更新并返回 `RUN_RECOVERY_FAILED`。其他拒绝或 publish 失败同样以 candidate tree 区分 Run 值与刚出现的外部值：撤回仍属于 Run 的文件，保留 staged/unstaged/untracked/ignored 外部内容；补偿不完整时保留 journal 与 Run ref 并返回 `RUN_RECOVERY_FAILED`。canonical index 的原子 rename 是最后一个可失败发布步骤，成功后不再执行会把已落库 Git 结果翻成失败的验证。branch switch 或 ref CAS 失败返回 `PROJECT_GIT_HEAD_UNAVAILABLE`，worktree/index 变化返回 `PROJECT_GIT_DIRTY`。不得自动 reset、clean、覆盖或吸收外部变化。
...
- 在 `read-tree` 前后、ref commit confirmation、ref publish 后和 index publish 前注入 staged、untracked、同路径外部写入与 publish failure；确认读取失败也按 attempted/unknown 重新检查精确 ref，拒绝路径撤回 Run 文件/ref、canonical index 保持准入状态，外部内容完整保留。
...
- Run 同时改变 `.gitattributes` 与受 `ident` filter 影响的同层文件，并在 ref publish 后失败；全部 candidate ownership 在任何删除前冻结，原 ref/index/worktree 完整恢复且 journal 无残留。
