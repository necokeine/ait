...
5. 最终整合采用带补偿日志的锁定发布协议而非“检查后再 merge”：先完成可失败的隔离 worktree 清理，再 prepare 带旧 OID 的 Git ref transaction；attached HEAD 固定并直接更新准入时的精确 symbolic target ref，detached HEAD 固定为 no-deref `HEAD`，后续 branch switch 不得改变补偿目标。取得 canonical index lock 并复制准入 index，同时按 baseline/candidate 两棵 tree 的精确 entry/type 把会被 Run 改写的路径和目标 index 记录到 manager-owned rollback journal；D/F 转换在补偿时先由深到浅撤下仍属于 Run 的 candidate entry，再由浅到深恢复 baseline entry。协议在 index lock 前、`read-tree` 前后、ref commit confirmation 前、ref publish 后及 index publish 前设置明确检查点，并反复校验 HEAD、branch、canonical index 原始字节、candidate index tree、tracked worktree 和 untracked files。ref commit 命令一经尝试即视为结果不确定；先完全结算 transaction 进程，再读取原始精确 target ref：仍为 candidate 时以旧 OID CAS 撤回，已为 baseline 时无需处理，其他 OID/identity 则保留外部更新并返回 `RUN_RECOVERY_FAILED`。其他拒绝或 publish 失败同样以 candidate tree 区分 Run 值与刚出现的外部值：撤回仍属于 Run 的文件，保留 staged/unstaged/untracked 外部内容；补偿不完整时保留 journal 与 Run ref 并返回 `RUN_RECOVERY_FAILED`。canonical index 的原子 rename 是最后一个可失败发布步骤，成功后不再执行会把已落库 Git 结果翻成失败的验证。branch switch 或 ref CAS 失败返回 `PROJECT_GIT_HEAD_UNAVAILABLE`，worktree/index 变化返回 `PROJECT_GIT_DIRTY`。不得自动 reset、clean、覆盖或吸收外部变化。
