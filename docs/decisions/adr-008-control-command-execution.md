...
3. 内部 `CommandOutcome` 区分已就绪结果与待执行 workspace Run 的 ID。仅新建 Codex Run 的 `SendMessage`、`ForkSession` 和 `TriggerCron` 分支产生执行指令；`RunView.status` 不用于调度。
...
6. 按明确的产品约定，Manual 和等待审批模式继续返回可查询、可取消的中间状态。查询返回当前快照；Cron 幂等重放返回已有 Run，即使它仍处于 queued，也不隐式恢复执行。后续恢复应由显式恢复操作或 worker 协议承担。
7. `apply_run_mode` 只处理本地模式的状态变化。workspace Agent 仍由 `WorkspaceAgent` port 注入，queued/running/terminal 三个持久化检查点保持原有边界。领域 Run 的终止屏障、Message 不可变性和 Session 指针语义不变。
