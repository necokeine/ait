...
3. 进度写入使用容量 256 的有界通道，以最多 64 项或 40ms 为一批写入独立 durable event outbox，
   ...
   它不逐 token 改写 Workspace 实体记录，也不修改未完成的 Message；最终 assistant Message 与
...
4. `/v1/event/stream` 先按 cursor 分页回放，再持续查询同一 durable outbox。回放和监听没有两个数据源，
   ...
   `stream.reset_required`，客户端直接采纳服务端 cursor 并重新读取 Project 范围实体与 active Run checkpoint。
...
5. Electron main 只建立一条受控事件流，并通过固定 IPC channel 广播给 renderer。每次文档加载由 preload
   ...
   与 ACK 也以 generation 隔离。ready 时主动同步当前连接状态并要求 entity/checkpoint resync；ACK 超时则
   舍弃未确认的临时投影并再次收敛到 resync。每个窗口至多有一帧未确认 IPC，main 与实体刷新期间的
...
6. provider turn 完成后尽力把 Run 展示为 `settling`；该中间状态写入失败不能阻断最终 Message 和 Run
   ...
   `run.cancelled`，收到两者后都重新读取 Project 范围实体，以不可变 Message 或终态卡片替换临时投影。
...
- desktop 压力与竞态测试覆盖 50,000 事件慢消费者、有界 frame/backlog、future cursor 收敛、
  ...
  旧 generation 迟到 ACK、断线状态重同步、ACK 丢失后的超时恢复，以及 active view 收到取消
