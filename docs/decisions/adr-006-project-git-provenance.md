...
Message 树记录交互历史，但此前不能回答“这条用户输入基于哪个代码版本”。同时 Project 只保存本地目录，无法保留成员声明的远端仓库地址。若在有未提交变更时接受输入，单一 HEAD 也不足以复现当时工作区。
...
3. `Project.repo_url` 是可选的声明型来源字段。它不参与 Project 身份，也不取代实际 Git remote 的刷新与校验。
