...
4. 所有带 `follow_session_id` 的 Run 从 Session worktree 捕获 HEAD/index 基线。Codex 的 Session-bound invocation、API Provider 宿主工具、原生审批路径和 Session 标题生成均使用 Session worktree 作为 cwd 与 workspace 边界。NEC-304 起 Cron occurrence 创建并跟随新 Session；仅旧版已持久化的无 Session Run 继续以 Project 工作区为入口。
