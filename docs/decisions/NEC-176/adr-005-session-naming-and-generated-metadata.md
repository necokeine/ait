...
- 状态：Accepted，已实现
- 日期：2026-09-05
- 修订：ADR-001 v4 的 Session 展示元数据；经 NEC-301 改为使用可配置 Small Agent
- 来源：NEC-176
...
4. 临时标题显示后，桌面端异步触发一次只读标题请求。该请求最多读取清洗后的 2,000 字符，使用全局 Small Agent（未配置时回退 Default Agent）及 `title + description` JSON schema；旧数据两项均为空时兼容回退当前 Session Agent。
...
application 只依赖 `SessionTitleGenerator` port，并解析 Small Agent 的配置、Provider 与不透明凭据引用；adapter 负责只读调用、结构化输出与结果校验。标题生成不创建 Message、Run 或 Git 提交，也不能修改 Project 工作目录。NEC-301 新增的 Agent system prompt 只持久化，不进入该请求。
...
- Codex protocol/adapter 测试覆盖所选 Small Agent 的 model/reasoning、只读沙箱、结构化 schema、system prompt 不注入和中文工单标题。
