...
6. Run 是“一个 Session Message + 一个 Agent”的一次完整任务运行。内部 Agent 调用可以重试、压缩上下文、从检查点恢复并处理运行期间新入队的工作，但这些都不创建新 Run。
7. Provider/SDK Adapter 是 Agent 的实现细节，不作为 MVP 的一级领域实体。
8. Cron 固定引用一个 Session Message 和一个 Agent，按照时间安排反复启动 Run；它不新建 Session，也不隐式追随可变的 active head。
...
宿主 Run 协调器校验并持久化每个 ProposedMessage；Agent 不能绕过领域规则直接修改 Session 树。遇到 assistant Message 中的 ToolUse sub-message 时，宿主通过 tool bridge 执行工具，并把结果持久化为一个 ToolResult user Message，再将更新后的路径交还 Agent。
...
一个 Session 可以在不同 Run 中选择不同 Agent；不提供“修改 Session 的历史 Agent”这一操作。每个 Run 从创建起固定 `base_message_id`、`agent_id` 和 `agent_revision`，其生成的 Message 通过 `run_id` 获得完整来源。
...
Cron 的 target 就是固定的 `session_id + base_message_id + agent_id`。每次到点直接以该 Message 为基准、以该 Agent 启动一个新 Run；不自动创建输入 Message，不新建 Session，也不读取 `active_head_message_id`。若希望定时任务从另一个节点继续，必须显式更新 Cron 的 `base_message_id`。

触发时解析并快照 Agent 当前启用的 revision 到 Run。触发幂等键为 `cron_id + scheduled_at`；`concurrency_policy` 只约束同一 Cron 产生的 Run。Cron 引用的 Session、Message 或 Agent 不可用时，本次触发失败并记录稳定错误，不静默换目标。
...
11. `completed` 只能由终止屏障原子写入：无待处理工具、重试、压缩/恢复或队列项，输出已落盘，且 `queue_version` 未变化。
12. Cron 必须固定引用 `session_id + base_message_id + agent_id`；每次触发创建新 Run，按 dedupe key 幂等。多个触发从同一 Message 形成分支，不能静默覆盖 active head。
...
queued
  ...
  -> settling
       ...
       -> completed            // 终止屏障 CAS 成功

任一非终态 -> cancelled | limit_exceeded
不可恢复错误或重试耗尽 -> failed
```

Agent 产生最终 assistant Message 且没有待处理 ToolUse 时，只能进入 `draining_queue/settling`，不能直接进入 `completed`。可重试错误进入 `retry_wait`；压缩或恢复只是同一 Run 的内部 phase。用户取消为 `cancelled`；预算或步数达到上限为 `limit_exceeded`；不可恢复错误或重试耗尽为 `failed`。`waiting_approval` 可恢复到 `executing_tool`，也可因拒绝先持久化 denied ToolResult user Message 再回到 `calling_agent`。
...
createCron(session_id, base_message_id, agent_id, schedule, timezone, policies)
```

`resumeRun` 恢复同一个 Run，不重复已提交 Message 或已完成工具调用。`enqueueRunItem` 与终止屏障以 `queue_version` 协调。所有创建型接口接受可选 idempotency key。
...
- Cron：`CRON_SESSION_UNAVAILABLE`、`CRON_BASE_MESSAGE_UNAVAILABLE`、`CRON_AGENT_UNAVAILABLE`、`CRON_DUPLICATE_FIRE`、`CRON_CONCURRENCY_BLOCKED`
...
4. **Cron target**：固定 `session_id + base_message_id + agent_id`。每次到点从同一 Message 配合该 Agent 创建新 Run；不新建 Session、不添加隐式输入、不追随 active head。
5. **本地持久化**：SQLite + append-only Message + FTS5；sub-message 随 Message 原子持久化；大附件采用内容寻址文件存储；schema 使用单调递增 migration，并在迁移前备份数据库。

## 9. 实现备注

此 ADR 固化领域边界，不固定语言、ORM 或 provider SDK。实现阶段若发现 Provider 无法无损映射到上述 Message/sub-message 协议，应新增兼容性 ADR，而不是增加 Message role、把 ToolUse 升格为 Session 节点，或原地修改历史 Message。实现 Run supervisor 时，Agent 调用结束不得直接写 `completed`；必须统一经过终止屏障。
