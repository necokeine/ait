...
本阶段接通 Forge、PR 与检查状态分组的 10 个规范 WebSocket 方法。经第十一阶段按生产 capability 集合与
catalog 去重重算，本阶段完成时实际发布 93 个方法：Paseo 清单内 82 个，独立 server 原有方法 11 个。
191 个 Paseo 来源名合并为 188 个独立方法，因此本阶段结束时剩余 106 个未接通。原先的 81/110 统计混用了
阶段新增数量和来源条目数，已在此更正。
