...
必须位于实际隔离工作区，`..`、越界或无法解析的符号链接被拒绝。当前通用 shell 审批无法证明
执行仍受 sandbox 约束，因此仅明确选择 `full_access` 的新 Run 可批准；网络目的地审批独立处理。
拒绝后可继续选择 deny/cancel，不能通过 session scope 绕过 Run 或管理员上限。
