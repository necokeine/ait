...
- 启用后第一次触发原子创建一个 Run 和一个新 Session；Run 为 `trigger=cron`、
  `cron_id=cron-daily`，且 `session_id` 指向新 Session。`base_message_id` 固定为配置中的节点，
  不自动插入 user 输入；结果推进新 Session。
- 重复 `cron_id + scheduled_at` 返回原 Run，不增加 Session、Message、Run 或 durable event。
- 不同 occurrence 产生不同 Run 与 Session，但都从同一固定基点形成分支；已有 Session 指针和版本不变。
