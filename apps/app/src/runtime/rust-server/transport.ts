  ...
  return ({ url, headers }) => {
    ...
    function receive(channel: number, data: unknown, binary: boolean): void {
      ...
      if (message.type === "server_info") {
        if (!helloSent || negotiated.has(channel)) throw new Error("Unexpected Rust server hello");
        ...
        cleanup.push(
          ...
          channel.onMessage((data, binary) => {
            try {
              ...
              fail(error instanceof Error ? error : new Error("Invalid Rust server response"));
            }
          }),
          channel.onError((error) =>
            fail(
              error instanceof Error
                ? error
                : new Error("Rust server transport failed; check address and Bearer token"),
            ),
          ),
    ...
    return {
      send(data) {
        if (disposed) throw new Error("Rust server connection is closed");
