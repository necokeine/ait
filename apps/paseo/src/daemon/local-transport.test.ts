...
describe("Remote SSH desktop transport", () => {
  it("accepts only token-free loopback Rust endpoints", () => {
    ...
    for (const url of [
      ...
      "ws://token@127.0.0.1:7316/v1/ws",
      "ws://127.0.0.1:7316/v1/ws?token=secret",
    ]) {
      expect(() => parseTransportTarget({ transportType: "rustTcp", url })).toThrow();
    }
  });

  it("sets Authorization on the main-process socket and preserves JSON text buffers", async () => {
    const socket = createConnectingSocket();
    const h = createManagerHarness(
      async () => ({ ...createEndpoint(), url: "ws://127.0.0.1:7316/v1/ws" }),
      [socket],
    );
    const target = { transportType: "rustTcp", url: "ws://127.0.0.1:7316/v1/ws" };
    expect(() => h.manager.open({ sessionId: "bad", target, bearerToken: "short" })).toThrow(
      "Bearer token",
    );
    h.manager.open({ sessionId: "rust", target, bearerToken: "t".repeat(32) });
    await Promise.resolve();
    expect(h.createWebSocket).toHaveBeenCalledWith(target.url, {
      headers: { Authorization: `Bearer ${"t".repeat(32)}` },
    });
    const opened = (socket.once as ReturnType<typeof vi.fn>).mock.calls.find(
      ([event]) => event === "open",
    )![1];
    opened();
    const message = (socket.on as ReturnType<typeof vi.fn>).mock.calls.find(
      ([event]) => event === "message",
    )![1];
    message(Buffer.from('{"type":"server_info"}'), false);
    expect(h.events.at(-1)).toEqual({
      sessionId: "rust",
      kind: "message",
      text: '{"type":"server_info"}',
    });
    h.manager.closeAll();
  });
