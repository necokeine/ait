...
export interface RustTcpTransportTarget {
  transportType: "rustTcp";
  url: string;
}

export type TransportTarget = LocalTransportTarget | SshTransportTarget | RustTcpTransportTarget;
...
function describeTransportTarget(target: TransportTarget): string {
  if (target.transportType === "rustTcp") return "Rust server";
...
export function parseTransportTarget(value: unknown): TransportTarget {
  ...
  if (value.transportType === "rustTcp") {
...
function parseOpenTransportSessionInput(value: unknown): OpenTransportSessionInput {
  ...
  if (target.transportType === "rustTcp") {
    ...
    return { sessionId, target, authorization: `Bearer ${value.bearerToken}` };
  }
...
async function resolveTransportEndpoint(target: TransportTarget): Promise<TransportEndpoint> {
  if (target.transportType === "rustTcp") {
    return { url: target.url, close: () => {}, failureDetail: () => null };
  }
  if (target.transportType === "ssh") {
...
export function createLocalTransportManager(
  ...
  async function connectSession(session: Session): Promise<void> {
    ...
    try {
      ws = session.authorization
        ? deps.createWebSocket(endpoint.url, { headers: { Authorization: session.authorization } })
        : deps.createWebSocket(endpoint.url);
    } catch (error) {
      failOpeningSession(
        session,
        `Failed to connect to ${describeTransportTarget(session.target)}: ${getErrorMessage(error)}`,
  ...
  function open(rawInput: unknown): void {
    const { sessionId, target, authorization } = parseOpenTransportSessionInput(rawInput);
